...
| `src/components/settings/history/HistorySettings.tsx` | A second tab showing the post-processed text the database has always stored and this page never displayed, plus the button that regenerates it, and one line rendering the fork's export button          |
| `src-tauri/src/input.rs`                              | `with_modifiers_held` and the `KeyInjector` seam, so a failed key injection cannot leave a modifier held down at the OS                                                                                  |
| `src-tauri/src/clipboard.rs`                          | `send_return_key`'s two modifier chords routed through that same helper                                                                                                                                  |
