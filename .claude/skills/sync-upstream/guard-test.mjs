...
const MUTATIONS = [
  ["upstream post-processing still has a caller",
    "src-tauri/src/actions.rs", "post_process_transcription(&settings"],
  ["post-processing gates on the rotation, not just the toggle",
    "src-tauri/src/cloud/post_process.rs", "cloud_bindings.post_process.entries.is_empty()"],
  ["local model load is deferred when cloud serves speech",
    "src-tauri/src/actions.rs", "if !cloud_stt_enabled {"],
  ["the fallback path loads the model before transcribing",
    "src-tauri/src/actions.rs", "tm.initiate_model_load();"],
  ["cloud transcripts get the local text cleanup",
    "src-tauri/src/actions.rs", "apply_text_post_processing"],
  ["streaming is suppressed while cloud speech is on",
    "src-tauri/src/actions.rs", "let model_supports_streaming = !cloud_stt_enabled"],
  ["Apple Intelligence is reachable through the pool",
    "src-tauri/src/cloud/post_process.rs", "apple_intelligence_completion"],
  ["cloud STT errors go through the URL sanitizer",
    "src-tauri/src/cloud/stt.rs", "report_reqwest_error"],
  ["transport failures are classified non-striking",
    "src-tauri/src/cloud/error.rs", "FailureClass::Unreachable"],
  ["the save path de-duplicates on credential AND model",
    "src-tauri/src/cloud/binding.rs", "seen.insert((entry.credential_id"],
  ...
  ["the cloud module is declared",
    "src-tauri/src/lib.rs", "pub mod cloud;"],
  ["the degraded event is registered",
    "src-tauri/src/lib.rs", "CloudDegradedEvent"],
  ["all 7 fork commands are registered",
